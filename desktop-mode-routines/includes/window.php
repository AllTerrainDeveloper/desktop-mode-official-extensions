...
function wpdm_routines_register_assets() {
	...
	$js_path  = DMR_DIR . $src;
	$css_path = DMR_DIR . 'assets/css/routines.css';
	...
	wp_register_style(
		'wp-desktop-routines',
		DMR_URL . 'assets/css/routines.css',
		array(),
		file_exists( $css_path ) ? (string) filemtime( $css_path ) : DMR_VERSION
	);
}
...
function wpdm_routines_localize_config() {
	...
	wp_localize_script(
		...
		array(
			...
			'pluginUrl'       => esc_url_raw( rtrim( DMR_URL, '/' ) ),
		)
	);
